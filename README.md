...
学生模式读取 `configs/teaching/yucai.json` 的课程名称和语言。“连接并检查手环”已接入基础设备检查；“设置游戏指令”“训练和优化我的模型”明确标记为“后续课程开放”且不可点击；其他入口页面明确标记为“功能接入中”，尚未接入采集、识别、训练或挑战赛业务流程。

### 学生模式：连接并检查手环

在学生模式中进入“连接并检查手环”，点击“连接并检查手环”即可开始异步检测；完成后可点击“重新检测”。页面只显示左右手连接状态、基础信号状态和数据接收稳定性，不显示端口、波特率、包统计或调试地址。

- 单手或双手均可检查；只要至少一侧通过检查，才可进入“采集我的手势”的占位页。
- 当前协议与串口枚举没有左右手身份字段，且教师端没有可复用的持久化映射，因此学生模式不会按端口顺序猜测左右手。真实设备未提供可靠侧别映射时，会提示“无法确认左右手，请老师在教师模式中完成分配”。
- 基础信号检查要求：持续收到至少 50 个有限的 8 通道 EMG 样本；任一通道不可长期平直（标准差至少 `0.5 µV`）；幅度不能超过 `20000 µV`；EMG 速率需在 `200–300 SPS`，且观测期内速率波动不超过 `40 SPS`。这只是基础数据质量筛查，并不能判断医学意义上的电极接触质量。
- 使用 `--mode student --simulate` 时，检测页会明确显示“演示模式”，结果来自模拟数据，不能代表真实手环已经通过检查。

该检测也是学生采集流程的前置门禁；未通过检查不能创建正式采集会话。

### 学生模式：采集我的手势

“采集我的手势”现已接入引导采集流程。先完成手环检查，再输入匿名学生/小组编号（仅字母、数字、下划线、短横线；不得使用真实姓名），选择已通过检查的左手或右手，以及每类 `5`、`10` 或 `15` 次。仅一侧通过时会自动锁定该侧；两侧均通过时可自行选择。

课程动作、中文提示、时序和随机顺序均来自 `configs/teaching/yucai.json`。学生只看到自然语言提示、倒计时和进度，可暂停/继续、重做或提前结束；系统会在动作阶段检查样本数量、有限值、全零/平直数据和最低采样比例，失败的试次不会计入有效完成数。

会话会自动创建在 `data/datasets/<匿名编号>/<自动会话ID>/`，不会覆盖已有数据。`metadata.json` 保存匿名编号、手侧、课程 ID、动作顺序、课程配置、创建/完成时间、采集状态和试次状态；正常完成、部分完成和异常中断会被明确区分。当前仍未接入学生侧训练、推理、游戏或结果分析页面；真实手环采集需要另行人工验收。
